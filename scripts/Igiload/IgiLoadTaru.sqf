...
IL_Taru_Parachute_Open_Altitude = 75;
        ...
        waituntil
        {
...

            {
                if (isnil {_x getVariable "IL_Taru_Action_Attach"}) then
                {
                    ...
                    IL_Taru_Action_Attach = _x addAction ["<img image='scripts\IgiLoad\images\load.paa' /><t color=""#007f0e""> Attach the Pod</t>", "[""attach"",_this] call IL_Taru_Do_Action;", nil, 2, false, true, "",
                ...
                if (isnil {_x getVariable "IL_Taru_Action_Deattach"}) then
                {
                    ...
                    IL_Taru_Action_Deattach = _x addAction ["<img image='scripts\IgiLoad\images\unload.paa' /><t color=""#ff0000""> Detach the Pod</t>", "[""deattach"",_this] call IL_Taru_Do_Action;", nil, 2, false, true, "",
                ...
                if (isnil {_x getVariable "IL_Taru_Action_Drop"}) then
                {
                    ...
                    IL_Taru_Action_Drop = _x addAction ["<img image='scripts\IgiLoad\images\unload_para.paa' /><t color=""#b200ff""> Drop the Pod</t>", "[""drop"",_this] call IL_Taru_Do_Action;", nil, 2, false, true, "",
    ...
    IL_Taru_Transmission =
    {
        ...
        switch (_args select 0) do {
            ...
            case 'chat_drop_without_parachute': {
                 if (!(player in crew _helico)) exitWith{};
                 ["Success",["Pod drop without Parachute!"]] call ExileClient_gui_notification_event_addNotification;
            };
        };
    };
...
if (isDedicated || isServer) then {
    ...
    IL_Taru_Attach_Pod =
    {
        ...
        if (!isTouchingGround _helico) then
        {
            ...
            [_helico] call IL_Client_Control;
